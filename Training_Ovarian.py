...
print('Load Ovarian Dataset ...')
# temp = np.load('Preprocessed Dataset/Without MI/Data_minmax_Ovarian_Full.npy')
temp = np.load('Preprocessed Dataset/With MI/Ovarian/MI_0.1_Ovarian_Full.npy')
...
dbn = DBN(hidden_layers_structure=[100,20],
          learning_rate_rbm=0.02,
          learning_rate=0.02,
          n_epochs_rbm=3000,
          n_iter_backprop=3000,
          batch_size=20,
...
for train_index, test_index in kf.split(listData):
    ...
    location = 'Model/Skenario 4/Ovarian/S4-100-20 MI 0.1 LR 0.02 K-'+str(k+1)+'.npy'
    k = k+1
